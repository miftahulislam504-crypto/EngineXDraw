...
import type { LibraryCategory } from '@archibim/object-model';
...
export const seedLibraryDefaults = onCall(async (request) => {
  ...
  const existingSnap = await db.collection('libraryItems').where('isCustom', '==', false).get();
  const existingKeys = new Set(existingSnap.docs.map((d) => `${d.data().category}:${d.data().name}`));
  ...
  for (const item of STARTER_ITEMS) {
    ...
    const ref = db.collection('libraryItems').doc();
    batch.set(ref, { ...item, isCustom: false, createdAt: FieldValue.serverTimestamp() });
    insertedCount++;
  }
  if (insertedCount > 0) {
    await batch.commit();
  ...
  return { insertedCount, alreadySeeded: insertedCount === 0 };
});
